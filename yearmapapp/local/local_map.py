...
class Local_map :
    ...
    def initDataFrame(self):
        file_path = "./yearmapapp/local/전력량_지방세_마커.csv"
    ...
    def setDataFrame(self, year_data) :
        ...
        self.df_data = self.df_ver[['시도', '사용량', '지방세', '위도', '경도']]
        self.df_data_test = self.df_data.to_dict(orient='records')

    def initJson(self) :
        jsonfile = open('./yearmapapp/local/Sido.json','r',encoding='utf8').read()     
        ...
        for idx in self.jsondata['features']:
            ...
            for i in self.df_data_test:
                if i['시도'] == idx['properties']['CTP_KOR_NM']:    
                    idx['properties']['pop'] = i['지방세']         
            self.jsondata_pick.append(idx)                               
        self.jsondata_loc['features'] = self.jsondata_pick 

    def map_base(self) :
        self.local_map = folium.Map(location=[36.0068191, 127.6607805],zoom_start=7) 
        cho = folium.Choropleth(geo_data=self.jsondata_loc,              
                        data=self.df_data,                          
                        columns=['시도', '지방세'],              
                        fill_color="YlGnBu",
                        key_on='feature.id',
                        legend_name='맵').add_to(self.local_map)       
        ...
        return self.local_map._repr_html_()
